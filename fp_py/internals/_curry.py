...
def get_placeholder_pos(*args: any) -> tuple[bool]:
    '''Returns a tuple of booleans indicating if the arguments are placeholder_pos'''
...
def _curry_2(fn: callable)->callable:
    ...
    def curried_fn(*args):
        ...
        if placeholder_pos.count(False) == 2:
            return fn(*args)
        # all placeholder_pos so return the curried function
        if placeholder_pos.count(True) == 2:
            return curried_fn
        return get_fn(len(args), placeholder_pos, *args)
...
def _curry_3(fn: callable)->callable:
    ...
    def get_fn(arg_count, positions, x=None, y=None, z=None):
        return {
    ...
    def curried_fn(*args):
        ...
        if placeholder_pos.count(False) == 3:
            return fn(*args)
        if placeholder_pos.count(True) == 3:
            return curried_fn
        if placeholder_pos.count(True) == num_args - 1:
            return _curry_2(get_fn(num_args, placeholder_pos, *args))
        return get_fn(num_args, placeholder_pos, *args)
